let { readDataFromFile } = require('./fileIO.js');
let { PATH_TO_CUSTOMERS_FILE } = require('../../config.json');

describe("Read Data from File", () => {
	test("read data and compare with expectedOutput", () => {
		...
		return expect(readDataFromFile(PATH_TO_CUSTOMERS_FILE)).resolves.toEqual(expectedOutput);
	})

	test("should be rejected, file path undefined", () => {

		return expect(readDataFromFile()).rejects.toThrow('Received type undefined');
	})

	test("should be rejected, file not found", () => {

		return expect(readDataFromFile('../wrongpath.txt')).rejects.toThrow('no such file or directory');
	})
});